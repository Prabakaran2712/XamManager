...
exports.login=async (req,res)=>{
    try{
        ...
        const users=await students.findOne({email:req.body.email},{_id:0,__v:0}).then((user)=>{
...
exports.update=async (req,res)=>{
    try{
        ...
        const upstudent=await students.findOneAndUpdate({rollNo:req.params.id},req.body,{
            new:true,
            runValidators:true
        });
        if(upstudent!=null){
...
exports.getHallticket=async (req,res)=>{
    try{
        const student=await students.findOne({rollNo:req.body.rollNo},{hallTicket:1});
        if(student!=null){
            console.log("retrieving...");
            res.set("Content-Type",student.hallTicket.contentType);
            res.status(200).send(student.hallTicket);
        }
        else{
...
exports.getNotifications=async (req,res)=>{
    try{
        const list=await notifications.find({deptID:req.body.deptID},{_id:0,__v:0});
        if(list!=null){
...
exports.verifyEmailConfirmation=async (req,res)=>{
    try{
        ...
        const verified=await otps.findOne({otpID:req.body.rollNo,otp:req.body.otp});
        console.log(verified);
        if(verified!=null){
            const verstudent=await students.findOneAndUpdate({rollNo:req.body.rollNo},{verified:true},{
                new:true,
                runValidators:true
            });
            if(verstudent!=null){
