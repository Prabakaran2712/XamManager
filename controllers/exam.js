...
exports.getExam = async (req, res) => {
  try {
    const data = await exams.findOne({ examID: req.params.id });
    if (data != null) {
      res.status(200).json(data);
    } else {
      res.status(200).json({
...
const updateHalltickets = async (student, data) => {
  try {
    ...
    htmlTopdf.generatePdf(file, options).then(async (pdfBuffer) => {
      ...
      const a = await students.findOneAndUpdate(
        { rollNo: student.rollNo },
        { hallTicket: pdfBuffer },
        {
          new: true,
          runValidators: true,
        }
...
exports.updateExam = async (req, res) => {
  try {
    const upExam = await exams.findOneAndUpdate(
